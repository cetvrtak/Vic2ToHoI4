#ifndef HoI4AISTRATEGY_H_
#define HoI4AISTRATEGY_H_



#include "../../V2World/AIStrategy.h"



namespace HoI4
{

class AIStrategy
{
	public:
		explicit AIStrategy(std::string strategyType):
			type(std::move(strategyType)),
			id(""),
			value(0)
		{}

		AIStrategy( std::string strategyType, const Vic2::AIStrategy& oldStrategy):
			type(std::move(strategyType)),
			id(oldStrategy.getID()),
			value(oldStrategy.getValue())
		{}

		void setID(const std::string HoI4Tag) { id = HoI4Tag; }
		void updateStrategy();
		void increaseValue(int valueToAdd) { value += valueToAdd; }

		[[nodiscard]] const auto& getType() const { return type; }
		[[nodiscard]] const auto& getID() const { return id; }
		[[nodiscard]] int getValue() const { return value; }

	private:
		std::string	type;
		std::string id;
		int value;
};

}



#endif // HoI4AISTRATEGY_H_